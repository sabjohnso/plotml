...
struct Write_curves_test {
  Write_curves_test() : accum( 0 ) { plotData_test(); }
...
private:
  ...
  plotData_test() {
    ...
    auto xminit =
        std::min_element( curve.d().begin(), curve.d().end(),
                          []( auto da, auto db ) { return da.x() < db.x(); } );

    auto xmin = xminit->x();

    auto xmaxit =
        std::max_element( curve.d().begin(), curve.d().end(),
                          []( auto da, auto db ) { return da.x() < db.x(); } );
    ...
    std::ifstream inp( std::string( data_path ) + "/" +
                       std::string( "curve1.xml" ) );

    std::string expected( ( std::istreambuf_iterator<char>( inp ) ),
